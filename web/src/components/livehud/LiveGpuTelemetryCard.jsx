...
export default function LiveGpuTelemetryCard({ gpuStats, job }) {
  const devices = gpuStats?.devices || [];
  const hasMulti = devices.length > 1;
  ...
  }, [devices.length, hasMulti]);

  const currentDev = hasMulti ? (devices[deviceIdx] || devices[0]) : gpuStats;
  ...
  const temp = Math.round(currentDev?.temp_celsius || 0);
  const power = Math.round(currentDev?.power_watts || 0);
  const util = Math.round(currentDev?.gpu_util_pct || 0);
  ...
  const clusterName = gpuStats?.model || 'NVIDIA GPU ACCELERATOR';

  return (
    <div className="cyber-card">
      <div className="card-header">
        <span className="card-title">3. HARDWARE & {clusterName.toUpperCase()}</span>
        {hasMulti ? (
          <div style={{ display: 'flex', gap: '0.25rem' }}>
            {devices.map((d, i) => (
              <button
                key={i}
                type="button"
                className={`cyber-pill ${deviceIdx === i ? 'active' : ''}`}
      ...
      <div className="telemetry-row">
        ...
        <span className="v" style={{ color: 'var(--cb-cyan)' }}>
          {currentDev?.model || clusterName} (GPU #{currentDev?.device_id || '0'})
        </span>
      </div>
      <div className="telemetry-row">
        <span className="k">GPU TEMP & POWER DRAW</span>
        <span className="v" style={{ color: temp < 65 ? 'var(--cb-green)' : 'var(--cb-yellow)' }}>
          {temp}°C • {power} W {hasMulti ? `(Total: ${Math.round(gpuStats?.power_watts || 0)}W)` : ''}
        </span>
      </div>
      <div className="telemetry-row">
      ...
      <div className="telemetry-row" style={{ borderBottom: 'none' }}>
        ...
        <span className="v" style={{ color: util > 30 ? 'var(--cb-yellow)' : 'var(--cb-green)' }}>
